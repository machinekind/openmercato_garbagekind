  ...
  async run(_rest) {
    ...
    const wynik = await em.getConnection().execute<Array<{ role_id: string }>>(
      `update dashboard_role_widgets d
          set widget_ids_json = d.widget_ids_json || '["edge.dashboard.liveness"]'::jsonb,
              updated_at = now()
        where d.deleted_at is null
          and not (d.widget_ids_json @> '["edge.dashboard.liveness"]'::jsonb)
          and exists (
            select 1 from role_acls a
             where a.role_id = d.role_id
               and a.deleted_at is null
               and (a.features_json @> '["edge.view"]'::jsonb
                 or a.features_json @> '["edge.*"]'::jsonb)
          )
        returning d.role_id`,
    )
    ...
    if (ile === 0) {
      console.log('Widget edge.dashboard.liveness: żadna lista nie wymagała zmiany.')
      console.log('Albo jest już dopisany, albo żadna rola nie ma uprawnienia edge.view.')
      return
    }
    console.log(`Widget edge.dashboard.liveness dopisany do list ról: ${ile}`)
    console.log('Widoczny po odświeżeniu pulpitu, w katalogu „Customize".')
  },
}

export default [issueCommand, simulateCommand, statusCommand, sweepCommand, installSchedulesCommand, installWidgetsCommand] satisfies ModuleCli[]
