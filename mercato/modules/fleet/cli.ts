  ...
  async run(_rest) {
    ...
    const wynik = await em.getConnection().execute<Array<{ role_id: string }>>(
      `update dashboard_role_widgets d
          set widget_ids_json = d.widget_ids_json || '["fleet.dashboard.readiness"]'::jsonb,
              updated_at = now()
        where d.deleted_at is null
          and not (d.widget_ids_json @> '["fleet.dashboard.readiness"]'::jsonb)
          and exists (
            select 1 from role_acls a
             where a.role_id = d.role_id
               and a.deleted_at is null
               and (a.features_json @> '["fleet.view"]'::jsonb
                 or a.features_json @> '["fleet.*"]'::jsonb)
          )
        returning d.role_id`,
    )
    ...
    if (ile === 0) {
      console.log('Widget fleet.dashboard.readiness: żadna lista nie wymagała zmiany.')
      console.log('Albo jest już dopisany, albo żadna rola nie ma uprawnienia fleet.view.')
      return
    }
    console.log(`Widget fleet.dashboard.readiness dopisany do list ról: ${ile}`)
